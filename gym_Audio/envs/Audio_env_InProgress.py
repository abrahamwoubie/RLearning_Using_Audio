...
class AudioEnv(gym.Env):
    ...
    def reset(self):
        start_state = np.argmax(self.isd)
        return start_state
    ...
    def step(self, action):
        ...
        transitions = self.P[self.s][self.a]
        p, s, r, d = transitions[0]
        self.s = s
        self.lastaction = self.a
        return (s, r, d, {"prob": p})

    def find_state_number(self,row, col):
        return row * self.ncol + col

    def find_rows_and_cols(self,row, col, a):
        if a == 0:  # left
            col = max(col - 1, 0)
        elif a == 1:  # down
    ...
    def Extract_Samples(self,row, col, letter):
